                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.business-settings.mail.update-sendgrid') }}" method="post">
                        ...
                        @if (isset($data_sendgrid))
                            ...
                            <div class="d-flex gap-10 align-items-center mb-4">
                                <input type="radio" name="status" value="0"
                                    {{ $data_sendgrid['status'] == 0 ? 'checked' : '' }}>
                                <label class="title-color mb-0">{{ \App\CPU\translate('Inactive') }}</label>
                            </div>
                            <div class="form-group">
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if (data.success === 2) {
                                    toastr.error(
                                        '{{ \App\CPU\translate('email_configuration_error') }} !!');
                                } else if (data.success === 1) {
                                    toastr.success(
                                        '{{ \App\CPU\translate('email_configured_perfectly!') }}!');
                                } else {
                                    toastr.info(
                                        '{{ \App\CPU\translate('email_status_is_not_active') }}!');
                                }
                            },
                            complete: function() {
                                $('#loading').hide();
