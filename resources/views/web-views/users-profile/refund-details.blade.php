...
@section('content')
    <div class="container text-center">
        <h2 class="headerTitle my-3" style=" font-size:33.23px; font-family: 'BURBANKBIGCONDENSED-BOLD' !important;">{{\App\CPU\translate('refund_request')}}</h2>
    </div>
<!-- Page Content-->
            ...
            <div class="card" style="border:none !important;">
                <div class="overflow-none">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-6 col-sm-6 col-md-2">
                                    <div>
                                        <img  class="__w-100px" onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'"
                                        src="{{\App\CPU\ProductManager::product_image_path('thumbnail')}}/{{$refund->product!=null?$refund->product->thumbnail:''}}"
                                        alt="VR Collection">
                                    </div>
                                </div>
                                <div class="col-6 col-sm-6 col-md-6">
                                    <span class="font-bold __text-18px" style="color:#FF061E;">
                                        @if ($refund->product!=null)
                                            <a style="color:#FF061E;" href="{{route('product',$refund->product->slug)}}">
                                                {{$refund->product->name}}
                                            </a>
                                        @else
                                    ...
                                    <span style="#000;">{{\App\CPU\translate('QTY')}} : {{$refund->order_details->qty}}</span><br>
                                    <span style="#000;">{{\App\CPU\translate('price')}} : {{\App\CPU\Helpers::currency_converter($refund->order_details->price)}}</span><br>
                                    @if ($order_details->variant!=null)
                                        <span style="#000;">{{\App\CPU\translate('variant')}} : </span>
                                        {{$order_details->variant}}
                                    @endif
                                </div>
                                <div class="col-12 col-sm-10 col-md-4  d-flex flex-column pl-0 mt-4 mt-sm-4 pl-sm-5">
                                    <div class="row justify-content-md-end mb-3">
                                        <div class="col-md-10 col-lg-10">
                                            <div class="row text-sm-right">
                                                <div style="#000;" class="col-sm-7">{{\App\CPU\translate('total_price')}} : </div>
                                                <div  class="col-sm-5 ">
                                                    <strong>{{\App\CPU\Helpers::currency_converter($refund->order_details->price*$refund->order_details->qty)}}</strong>
                                                </div>

                                                <div style="#000;" class="col-sm-7">{{\App\CPU\translate('total_discount')}} :</div>
                                                <div class="col-sm-5 ">
                                                    <strong>{{\App\CPU\Helpers::currency_converter($refund->order_details->discount)}}</strong>
                                                </div>

                                                <div style="#000;" class="col-sm-7">{{\App\CPU\translate('total_tax')}} :</div>
                                                <div class="col-sm-5">
                    ...
                    <div class="card mt-2">
                        <div class="card-body">
                            <div class="row ">
                                <span style="#000;" class="col-sm-2">{{\App\CPU\translate('subtotal')}} : {{\App\CPU\Helpers::currency_converter($subtotal)}}</span>
                                <span style="#000;" class="col-sm-5">{{\App\CPU\translate('coupon_discount')}} : {{\App\CPU\Helpers::currency_converter($coupon_discount)}}</span>
                                <span style="#000;" class="col-sm-5">{{\App\CPU\translate('total_refund_amount')}} : {{\App\CPU\Helpers::currency_converter($refund_amount)}}</span>
                            </div>
                        </div>
                    </div>
                    <div class="card mt-2">
                        <div class="card-body">
                            ...
                            <div class="col-12">
                                ...
                                <span class="text-capitalize "style="color:coral; font-weight:600;"> {{$refund->status}}</span>
                                @elseif($refund->status == 'approved')
                                <span class="text-capitalize" style="color:#058114; font-weight:600;"> {{$refund->status}}</span>
                                @elseif($refund->status == 'refunded')
                                <span class="text-capitalize" style="color:#1af0d3; font-weight:600;"> {{$refund->status}}</span>
                                @elseif($refund->status == 'rejected')
                                <span class="text-capitalize " style="color:#FF061E; font-weight:600;"> {{$refund->status}}</span>
                                @endif
                            </div>
